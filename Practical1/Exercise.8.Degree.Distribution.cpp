...
vector<pair<int,int>> computeDegreeDistribution(vector<pair<int,int>> nodes_degrees, int total_nodes){

    vector<int> degrees_in_a_graph;
    vector<pair<int,int>> node_degree_pair;

    for (int i =0;i < total_nodes;i++){

        degrees_in_a_graph.push_back(nodes_degrees[i].second);
    }

    std::sort(degrees_in_a_graph.begin(),degrees_in_a_graph.end());

    /*
    cout << "Degress moved to vector.\n";
    for (int j=0;j<degrees_in_a_graph.size();j++)
     {

        cout << "Node" << j << ": [" <<degrees_in_a_graph[j] <<"] \n";
     }

     */

    map<int,int> degreesCounter;

    //for_each(degrees_in_a_graph.begin(),degrees_in_a_graph.end(),[&degreesCounter](int counter){degreesCounter[counter]++;});

    for (auto x : degrees_in_a_graph) ++degreesCounter[x];

    for (auto j : degreesCounter){
    //    cout << "Node [" << j.first << "], Occurrences [" << j.second << "]\n";
        node_degree_pair.push_back(std::make_pair(j.first,j.second));
    }

    return node_degree_pair;

    /*
    cout << "Validating Before leaving the function that populates Nodes Distribution.\n";

    for (int i=0; i <node_degree_pair.size();i++)
    {
        cout << node_degree_pair[i].first << " " << node_degree_pair[i].second << "\n";
        //outputFile << node_dist_pair[i].first << " " << node_dist_pair[i].second << "\n";
    }
    */
    /*
     *
     * vector<pair<int, int>>
    int degree_frequency =0;
    int nodes_with_frequency =0;

    for (int k=0;k<degrees_in_a_graph.size();k++)
    {
        if (degrees_in_a_graph[i]==degrees_in_a_graph[i+1])
        {
            degree_frequency++;
        }
    }

         vector<pair<int, int>> degrees_distribution;
...
void savetoFileDegreeDist(vector<pair<int,int>> node_dist_pair){


    ofstream outputFile;
    outputFile.open("degree_dist.txt");
    ...
    if(outputFile.is_open()) {
        ...
        for (int i=0; i <node_dist_pair.size();i++)
        {
            //cout << node_dist_pair[i].first << " " << node_dist_pair[i].second << "\n";
            outputFile << node_dist_pair[i].first << " " << node_dist_pair[i].second << "\n";
        }
...
int main() {
    ...
    vector<pair<int, int>>node_dist_pair;
    ...
    */
    ...
    node_dist_pair = computeDegreeDistribution(nodes_degrees,total_nodes);

    savetoFileDegreeDist(node_dist_pair);
